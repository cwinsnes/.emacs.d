...
#+BEGIN_SRC emacs-lisp
  (use-package org
      :ensure nil
      :config
      ;; Due to some bug, these are not always set properly.
      ;; Re-set them here to avoid any issue.
      (unless (boundp 'org-priority-highest)
        (defvaralias 'org-priority-highest 'org-highest-priority))
      (unless (boundp 'org-priority-lowest)
